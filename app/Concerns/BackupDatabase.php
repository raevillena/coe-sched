...
trait BackupDatabase
{
    ...
    protected function performBackup(int $user_id)
    {
        try {
            ...
            $gzip_file_path = storage_path("app/backup_database/$gzip_file_name");

            // Ensure the backup directory exists
            if (!file_exists(storage_path('app/backup_database'))) {
                mkdir(storage_path('app/backup_database'), 0755, true);
            }
            ...
            //for windows
            //$gzip_path = "\"C:\\Program Files\\Git\\usr\\bin\\gzip.exe\"";
            //$backup_cmd = "\"C:\\xampp\\mysql\\bin\\mysqldump.exe\" --user=$username --password=$password --host=$host --port=$port --single-transaction --quick --lock-tables=false $database | $gzip_path > \"$gzip_file_path\"";
            //for ubuntu
            $backup_cmd = "mysqldump --user=$username --host=$host --port=$port --single-transaction --quick --lock-tables=false $database | gzip > \"$gzip_file_path\"";
